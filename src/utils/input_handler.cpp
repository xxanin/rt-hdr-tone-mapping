...
InputHandler::InputHandler(const FilterOptions &options) : m_options(options), m_frameCounter(0), m_isStaticImage(false) {
...
void InputHandler::displayFrame(const cv::Mat &frame) { cv::imshow("Filter Output", frame); }
...
void InputHandler::createGradient(cv::Mat &frame) {
  for (int y = 0; y < frame.rows; y++) {
    for (int x = 0; x < frame.cols; x++) {
      ...
      frame.at<cv::Vec3b>(y, x) = cv::Vec3b(value,              // Blue
...
void InputHandler::createNoise(cv::Mat &frame) {
  ...
  for (int y = 0; y < frame.rows; y++) {
    for (int x = 0; x < frame.cols; x++) {
      frame.at<cv::Vec3b>(y, x) = cv::Vec3b(dis(gen), // Blue
                                            dis(gen), // Green
                                            dis(gen)  // Red
      );
    }
